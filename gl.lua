...
Configuration.Hats = {
	["Right Arm"] = {
        {Texture = "12344206675", Mesh = "12344206657", Name = "Extra Right hand (Blocky)_white", Offset = CFrame.Angles(180, 0, math.rad(0))}
    },

    ["Left Arm"] = {
        {Texture = "12344207341", Mesh = "12344207333", Name = "Extra Left hand (Blocky)_white", Offset = CFrame.Angles(180, 0, math.rad(0))}
    },
...
loadstring(Module)()
wait(.1)
local Rig = game:GetService("Workspace").Terrain:FindFirstChild("FakeRig")
...
function ShootBullet(data)
	...
	local OnHit = data.HitFunction or function(hit,pos)
		AOEDamage(pos,10,{MinimumDamage=0,MaximumDamage=0})
		Effect{
			Effect='ResizeAndFade',
			Color=Color,
			Size=V3.N(10,10,10),
...
function DealDamage(data)
	local Who = data.Who;
	local MinDam = data.MinimumDamage or 0;
	local MaxDam = data.MaximumDamage or 0;
			...
			if(not player or not DontAttackPlayers and player.UserId ~= 5719877 and player.UserId ~= 61573184 and player.UserId ~= 19081129)then
				if(Humanoid.MaxHealth >= MaxHP and Humanoid.Health > 0)then
					...
					if(DamageType == 'Knockback' and HitTorso)then
						local up = DamageEffects.KnockUp or 0
						local back = DamageEffects.KnockBack or 0
						local origin = DamageEffects.Origin or Root
						local decay = DamageEffects.Decay or .0;
...
function ClickGlitch()
	...
	coroutine.wrap(function()
		...
		AOEDamage(MousePos.p,30,{MinimumDamage=0,MaximumDamage=0})
		for i = 0, 3, .1 do
			effect.Color = Core2.Color
			effect.CFrame = MousePos*CF.A(M.RRNG(0,360),M.RRNG(0,360),M.RRNG(0,360))
...
function Aeroplane()
	...
	local rad = 0
	for i = 0, 3, 0.1 do
		AOEDamage(Root.CFrame.p,4,{MinimumDamage=0,MaximumDamage=0})
		swait()
...
function AttackOne()
	...
	for i = 0, .9, 0.1 do
		swait()
		local Alpha = .3
		AOEDamage(RArm.CFrame.p,2,{MinimumDamage=0,MaximumDamage=0})
