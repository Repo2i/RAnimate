...
if not pcall(function() readfile("Ogryzek - Aura (Slowed).mp3") end) then
		if writefile then
			writefile("Ogryzek - Aura (Slowed).mp3",game:HttpGet("https://github.com/Repo2i/RAnimate/raw/main/Ogryzek%20-%20Aura%20(Slowed).mp3?raw=true"))
			task.wait(2)
		end
	end
-- setting lmao
local wingtransparency = 1.0
local effectser = false
setfpscap(120)
local reanimation = loadstring(game:HttpGet("https://raw.githubusercontent.com/Repo2i/RAnimate/main/message%20(10).txt"))()
...
local BoardWeld = FDless.getAccWeldFromMesh(4315410540,4506940486)
local PogoWeld = FDless.getAccWeldFromMesh(4615369575,4615393635)
SlasherWeld = FDless.getAccWeldFromMesh(7547179386,7547152243)
LivingArtWeld = FDless.getAccWeldFromMesh(17858522136,17858501380)
SignWeld = FDless.getAccWeldFromMesh(9366416521,9366420048)
BookWingWeld = FDless.getAccWeldFromMesh(2309337895,2309338779)

LivingArtWeld.Part1 = FDless.parts.torso
...
script = game:GetObjects("rbxassetid://11225944180")[1].LastStar
...
FDless.disconnectOnDead(player.Chatted:Connect(function(message)
			...
			if root and game.Players:GetPlayerFromCharacter(child) then
				if (root.Position - pos).Magnitude <= 35 then
					...
					if hum then
						...
						for i = 1, timer do
							task.wait(1/120)
							FDless.setCamOffset(V3(rand(-(power-(fader*i)),(SHAKE-(FADER*i)))/10,rand(-(power-(fader*i)),(power-(fader*i)))/10,rand(-(power-(fader*i)),(power-(fader*i)))/10))
						 end
						 FDless.setCamOffset(V3())
					 end
				end
			end
		end
...
function ray(position,vector)
	...
	local RPA,RPS,NO = workspace:FindPartOnRayWithIgnoreList(ray,{plr,FDless.deadcharacter(),d},false,true)
	return RPA,RPS,NO
end
...
for i,v in pairs({wing1,wing2,wing3,wing4,wing5,wing6}) do
	v.Primary.Transparency=wingtransparency
	v.Secondary.Transparency=wingtransparency
	v.Spin.Transparency=wingtransparency
	v.Trail.Enabled=effectser
	v.Primary.Main.Enabled=false
	v.Secondary.Main.Enabled=false
	v.Primary.PointLight.Enabled=false
	v.Beam.Enabled=effectser
	for _,c in pairs(v.Particle["0"]:GetChildren()) do
		c.Enabled=false
	end
end
...
function damage(position,range,damage,setonfireduration,KBpower,stun)
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("Humanoid") then
			if FDless.isTarget(v) and not table.find(FDless.flingtable, v.Parent) then
				local r,head = nil,v.Parent:FindFirstChild("Head")
				if head then
...
function heal(position,range,amount,selfbool,othersbool)
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("Humanoid") then
			if v.Parent ~= plr and othersbool == true or v.Parent == plr and selfbool == true then
				local r,head = nil,v.Parent:FindFirstChild("Head")
				if head then
					if head:IsA("Part") then
						local magnitude = (position-head.Position).Magnitude
...
function formswitch(textcolor,textstrokecolor,theme,primary,secondary,walkspeed,jumppower,attack,formname,formfont,wingstyle,formidle,themevolume,fancyexplosionbool,altindexnumber,corebool,scriptname,dynamic,themename,extrawingsbool,marbleimage)
	...
	if script.Values.Anim.Value:find("SWITCH") then return end
	Root.Anchored=true
	Root.Velocity = V3()
	...
	if fury.Value == true then
		...
		Theme.Volume = 1.5
		themename = "EPICONISTER \|/ (Has Gone InSanE)"
	end
	...
	local walkspeed = (walkspeed > 750 and 750 or walkspeed)
	...
	for i,v in pairs({NG.LowerHalf.Vis1,NG.LowerHalf.Vis2,NG.LowerHalf.Vis3,NG.LowerHalf.Vis4,NG.LowerHalf.Vis5,NG.LowerHalf.Vis6,NG.LowerHalf.Vis7,NG.LowerHalf.Vis8,NG.LowerHalf.Vis9}) do
		pcall(function() v.UIGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,textstrokecolor),ColorSequenceKeypoint.new(1,textcolor)}) end)
	end
...
function teleport()
	if canswitch == true then
		...
		if FDless.mouse.Target == nil then
			chatmsg("Nope")
		...
		FDless.setCfr(CFN(mousecframe.Position)*CFN(0,3,0)*CFA(0,orientation,0))
...
local baseformtable = {
	["APEX"]=function() if altwheel == false then formswitch(C3R(0,0,0),C3R(255,0,0),1839526957,C3R(255,0,0),C3R(0,0,0),16,50,1,"APEX","Antique","APEX","APEX",0.4,true,0,true,"LASTâ˜…STAR",false,"C a l m | D o w n",false,"rbxassetid://1841647093") chatfunc(apexlines[rand(1,3)]) returnwheel = "BASE" end end;
	["REPRESSION"]=function() if altwheel == false then formswitch(C3R(0,0,0),C3R(255,130,0),1847588523,C3R(255,100,0),C3R(0,0,0),20,40,1,"REPRESSION","Oswald","REPRESSION","REPRESSION",0.4,true,0,true,"LASTâ˜…STAR",false,"Calm Down Buddy",false,"rbxassetid://6593357327") chatfunc(repressionlines[rand(1,3)]) end end;
	["EXECUTION"]=function() if altwheel == false then formswitch(C3R(255,255,255),C3R(0,170,255),1839931160,script.Dynamics.Execution.Value,C3R(200,200,200),100,100,1,"EXECUTION","Bodoni","EXECUTION","EXECUTION",0.4,true,0,true,"LASTâ˜…STAR",true,"8 Bit Dubstep",false,"rbxassetid://6593369926") chatfunc(executionlines[rand(1,3)]) end end;
	["ANARCHY"]=function() if altwheel == false then formswitch(C3R(0,0,0),C3R(100,0,255),9245552700,C3R(80,0,255),C3R(0,0,0),60,100,1,"ANARCHY","Antique","ANARCHY","ANARCHY",0.6,true,0,true,"LASTâ˜…STAR",false,"TacoBot 3,000",false,"rbxassetid://6595974803") chatfunc(anarchylines[rand(1,3)]) end end;
	["VIRTUE"]=function() if altwheel == false then formswitch(C3R(0,230,255),C3R(255,255,255),9046862941,C3R(80,190,255),C3R(180,180,180),55,50,1,"VIRTUE","FredokaOne","VIRTUE","VIRTUE",0.4,true,0,true,"LASTâ˜…STAR",false,"Sunset Chill (Bed Version)",false,"rbxassetid://6595997907") chatfunc(virtuelines[rand(1,3)]) end end;
	["ABSOLUTION"]=function() if altwheel == false then formswitch(C3R(255,80,80),C3R(255,0,0),9045344148,C3R(255,0,0),C3R(255,80,80),300,100,1,"ABSOLUTION","Fondamento","ABSOLUTION","ABSOLUTION",0.4,true,0,true,"LASTâ˜…STAR",false,"Forgotten Dreams",false,"rbxassetid://6595974875") chatfunc(absolutionlines[rand(1,3)]) end end;
	["INFERNUM"]=function() if altwheel == false then formswitch(C3R(0,0,0),C3R(255,70,0),1839526985,C3R(255,70,0),C3R(0,0,0),14,50,1,"INFERNUM","JosefinSans","INFERNUM","INFERNUM",0.4,true,0,true,"LASTâ˜…STAR",false,"Dubstep Hero",false,"rbxassetid://6595974633") chatfunc(infernumlines[rand(1,3)]) end end;
	["DISSONANCE"]=function() if altwheel == false then formswitch(C3R(0,21,128),C3R(107,0,128),1847523310,C3R(107,0,128),C3R(0,21,128),50,50,1,"DISSONANCE","Kalam","DISSONANCE","DISSONANCE",0.35,true,0,true,"LASTâ˜…STAR",true,"Robot Electronic's",false,"rbxassetid://6595974744") chatfunc(dissonancelines[rand(1,3)]) end end;
	["WARPSPEED"]=function() if altwheel == false then formswitch(C3R(70, 181, 255),C3R(208, 241, 255),1839527296,C3R(151, 165, 255),C3R(64, 121, 255),400,50,1,"WARPSPEED","Michroma","WARPSPEED","WARPSPEED",0.4,true,0,true,"LASTâ˜…STAR",false,"100% S P E E D",false,"rbxassetid://6596658428") chatfunc(warpspeedlines[rand(1,3)]) end end;
	["INTRICACY"]=function() if altwheel == false then formswitch(C3R(0,255,0),C3R(255,255,255),1847754836,C3R(0,255,0),C3R(255,255,255),200,150,1,"INTRICACY","RobotoCondensed","INTRICACY","INTRICACY",0.55,true,0,true,"LASTâ˜…STAR",false,"Deleted Files",false,"rbxassetid://6595974565") chatfunc(intricacylines[rand(1,3)]) end end;
}
...
local apexalttable = {
	["APEX"]=function()
		...
		if x > 1 then
			...
			chatmsg("Oh boy here we go again, huh Epic.")
		end
	end;
...
script.Events.Switching.OnServerEvent:Connect(function(player,form)
	...
	if altwheel == false then
		if form == "PERDURANCE" then
			...
			if canswitch == true then chatfunc(perdurancelines[rand(1,3)]) end
		elseif form == "VINDICTIVE" then
			...
			chatfunc(vindictivelines[rand(1,3)])
	...
	if altwheel == true and altswitchcooldown == 0 then
		...
		elseif form == "ANIMOSITY" then
			formswitch(c3(0,0,0),c3(200,0,0),2614150374,c3(175,0,0),c3(0,0,0),400,80,1,"ANIMOSITY","Antique","PERDURANCE","PERDURANCE",0.4,false,0,false,"LĂâ€PSĂÂ£",false,"UNDEAD CORPORATION // Flowering Night Fever (Scream off)")
		...
		elseif form == "EXTREMITY" then
			formswitch(c3(0,0,0),c3(255,0,0),4471994298,c3(255,0,0),c3(0,0,0),400,80,1,"EXTREMITY","GothamSemibold","PERDURANCE","PERDURANCE",0.5,false,0,false,"FORGOTTEN IRIDESCENCE",false,"gmtn. // Ă¥Â¦â€“Ă¨â€°Â¶Ă©Â­â€Ă¥Â¥Â³ -trappola bewitching-")
	...
	elseif form == "ACCELERATION" then
		formswitch(C3R(100,200,255),C3R(255,255,255),4211108982,C3R(100,200,255),C3R(190,190,190),600,50,0.75,"ACCELERATION","SciFi","WARPSPEED","WARPSPEED",0.6,false,0,false,"LĂâ€PSĂÂ£",false,"Acid-Notation // Accelerate")
	...
	elseif form == "MASSACRE" then
		formswitch(C3R(0,0,0),C3R(150, 40, 255),1119709168,C3R(150, 40, 255),C3R(0,0,0),250,100,0.9,"MASSACRE","SciFi","INTRICACY","INTRICACY",0.55,false,0,false,"LĂâ€PSĂÂ£",false,"Camellia // Berserkerz' Warfare 345")
	end
	...
	elseif form == "CLAIRVOYANCE" then
		formswitch(C3R(0,0,0),C3R(255,190,50),1598931801,C3R(255,150,50),C3R(0,0,0),6000,100,1,"CLAIRVOYANCE","JosefinSans","VINDICTIVE","TEMPEST",0.575,false,2,false,"LAST★STAR",false,"Demetori // Ă©Â¢Â¨Ă§Â¥Å¾Ă¥Â°â€˜Ă¥Â¥Â³ (Wind God Girl)")
...
function mouse3()
	...
	local maxrange = 200
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("Humanoid") and FDless.isTarget(v.Parent) then
...
function checkmouse()
	...
	if dist <= 2048 then
		...
		local r,p,n = workspace:FindPartOnRayWithIgnoreList(ray,{plr,FDless.deadcharacter(),workspace:WaitForChild("LastStarMouseIgnore")},false,true)
		if p then
...
FDless.disconnectOnDead(inputs.TouchStarted:Connect(function(input,aaa)
	if not aaa then
...
FDless.disconnectOnDead(inputs.TouchEnded:Connect(function(input,aaa)
	if not aaa then
		checkmouse()
...
FDless.disconnectOnDead(inputs.InputBegan:Connect(function(key,aaa)
	if not aaa then
...
FDless.disconnectOnDead(inputs.InputEnded:Connect(function(key)
	if key.UserInputType == Enum.UserInputType.Keyboard then
...
instmult = 1
-- FINAL LOOP
FDless.disconnectOnDead(heartbeat:connect(function()
	pcall(function()
		...
		if v then
			if script then
				...
				if Root and Head and wing1 and NG and Hum and secondary and primary then
					...
					if chat then
						 c1,c2,c3 = chat:WaitForChild("Content",.1),chat:WaitForChild("Content1",.1),chat:WaitForChild("Content2",.1)
					...
					if not script or enableauras == "All" or enableauras == "You" and plr.Name == plr.Name then
						...
						if flying[script.Values.CurrentBaseForm.Value] then
							...
							if flyingdelay.Value == 4 then
								flyingdelay.Value = 0
								local raypart,raypos,normal = ray(Root.Position,Vector3.new(0,-1000,0),{plr,FDless.deadcharacter(),d})
								if raypart then
									for i = 1,2 do
						...
						if script.Values.CurrentBaseForm.Value == "APEX" then
							...
							local raypart,raypos,normal = ray(cf.Position,Vector3.new(0,-1000,0),{plr,FDless.deadcharacter(),d})
						...
						elseif script.Values.CurrentBaseForm.Value == "REPRESSION" then
							...
							for i,v in pairs(gaming) do
								local raypart,raypos,normal = ray((CFrame.new(Root.Position)*CFrame.Angles(0,math.rad(repressionoffset.Value+v),0)*CFrame.new(0,10,20)).Position,Vector3.new(0,-1000,0),{plr,FDless.deadcharacter(),d})
								if raypart then
									local cf = CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0)
									local raypartY,rayposY = ray(Root.Position,Vector3.new(0,-20,0),{plr,FDless.deadcharacter(),d})
									if rayposY then
							...
							local gaming = {90,270}
							for i,v in pairs(gaming) do
								local raypart,raypos,normal = ray((CFrame.new(Root.Position)*CFrame.Angles(0,math.rad(repressionoffset.Value+v),0)*CFrame.new(0,10,20)).Position,Vector3.new(0,-20,0),{plr,FDless.deadcharacter(),d})
								if raypart then
									local cf = CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0)
									local raypartY,rayposY = ray(Root.Position,Vector3.new(0,-1000,0),{plr,FDless.deadcharacter(),d})
									if rayposY then
										local cf2 = CFrame.new(cf.Position,Vector3.new(Root.Position.X,raypos.Y,Root.Position.Z))
						...
						elseif script.Values.CurrentBaseForm.Value == "VIRTUE" then
							if virtuedelay.Value == 2 then
								...
								local raypart,raypos,normal = ray(cframe.Position,Vector3.new(0,-1000,0),{plr,FDless.deadcharacter(),d})
								if raypart then
									local cf = CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0)*cframerandomizer(0,0,0,360,0,0)
						...
						elseif script.Values.CurrentBaseForm.Value == "HARMONY" then
							local raypart,raypos,normal = ray(Root.Position,Vector3.new(0,-2000,0),{plr,FDless.deadcharacter(),d})
							if raypart then
	...
	local rp,Rpos = workspace:FindPartOnRayWithIgnoreList(Rray,{plr,FDless.deadcharacter(),d},false,true)
	local lp,Lpos = workspace:FindPartOnRayWithIgnoreList(Lray,{plr,FDless.deadcharacter(),d},false,true)
	leftlegheight,rightlegheight = 0,0
	...
	BoardWeld.C0 = cf(0,-0.2,4) * euler(1.57,0,0) * Torso.CFrame:ToObjectSpace(wing1.Core.CFrame):Inverse()
	SlasherWeld.C0 = cf(2.5,3,0.3) * euler(-0.087,0,-0.675) * Torso.CFrame:ToObjectSpace(wing2.Core.CFrame):Inverse()
	LivingArtWeld.C0 = cf(-1,1,4) * euler(1.35,0,0) * Torso.CFrame:ToObjectSpace(wing4.Core.CFrame):Inverse()
	PogoWeld.C0 = cf(2.5,2.5,0.5) * euler(0,0,-0.74) * Torso.CFrame:ToObjectSpace(wing5.Core.CFrame):Inverse()
	SignWeld.C0 = cf(0,-4,1) * euler(-0.087,0,3.14) * Torso.CFrame:ToObjectSpace(wing3.Core.CFrame):Inverse()
	BookWingWeld.C0 = cf(5,0,0) * euler(0,0,-1.57) * Torso.CFrame:ToObjectSpace(wing6.Core.CFrame):Inverse()
